...
class Resnet50Train(Network):
    def build_graph(self):
        ...
        # (self.feed('conv_new_1_relu')
            ...
            # .conv3d_preserve(3, 3, 3, 256, 1, 1, 1, biased=False, relu=False,
                             # reduce_time=False, name='spacetime_conv1')
            # .batch_normalization(relu=False, name='spacetime_bn1')
            # .relu(name='spacetime_relu1')
            # .conv3d_preserve(3, 3, 3, 256, 1, 1, 1, biased=False, relu=False,
                             # reduce_time=True, name='spacetime_conv2')
            # .batch_normalization(relu=False, name='spacetime_bn2')
            # .relu(name='spacetime_relu2'))

        # Classification
        # (self.feed('spacetime_relu2')
        (self.feed('conv_new_1_relu')
    ...
    def compute_class_loss(self):
        ...
        cross_entropy_n = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=cls_score, labels=self.labels)
    ...
    def create_metrics(self):
        ...
        accuracy_vid, accuracy_vid_update = tf.metrics.accuracy(
                labels=self.labels, predictions=predictions_vid,
                name='accuracy_vid')
